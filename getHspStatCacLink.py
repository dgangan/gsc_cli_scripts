...
def main():
    ...
    pars_worker = CliParser(ParsingMap.hsp_stat_cac_link())


    #Creating TelnetCli connection objects for HSP and DPS
    ...
    #Getting 'cac stat global <link_id>' for all VSATs from HSP
    for vsat_id in vsat_ids:
        print (' -> Fetching VSAT: ' + vsat_id)
        hsp_telnet.send_command('stat cac link '+ vsat_id, by_symbol=True)
        raw_hsp_output = hsp_telnet.read_output()
        parsed_hsp_out = pars_worker.parse_output(raw_hsp_output, entry_id=vsat_id)
        pars_worker.aggregate_csv(parsed_hsp_out, datetime = DATETIME)
    print('\nCompleted: fetched: '+str(len(vsat_ids))+' VSATs\n')
    ...
    #If SPECIAL_OPTION enabled: clearing statistic at the end
    if SPECIAL_OPT:
